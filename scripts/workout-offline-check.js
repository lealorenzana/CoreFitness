/**
 * Offline sets (lib/offlineSets.ts) against the routine-run fixture: with the
 * connection dropped a set stays on the phone, the rest still starts, Finish is
 * refused, a reload keeps it, and when the signal returns it uploads once with
 * the id the phone gave it. Writes shots/offline-NN-*.png.
 *
...
async (page) => {
  ...
  const out = [];
  const shot = async (name) => page.screenshot({ path: `shots/offline-${name}.png` });
  const go = async (p) => {
  ...
  // Cleared once per test, not on every navigation — step 3 reloads on purpose.
  await page.addInitScript(() => { if (!sessionStorage.getItem('outbox-reset')) { localStorage.removeItem('cf:set-outbox:v1'); sessionStorage.setItem('outbox-reset', '1'); } });
  ...
  // 1 — no signal: the set stays on the phone and the rest still starts
  globalThis.__offline = true;
  await btn('Log set 1').first().click();
  await page.waitForTimeout(1200);
  out.push('server sets while offline: ' + DB.workout_sets.length);
  const rest = await page.getByRole('dialog', { name: 'Rest' }).count();
  out.push('rest started anyway: ' + (rest ? 'yes' : 'NO'));
  if (rest) await btn('Skip rest').click();
  await page.waitForTimeout(300);
  const outbox = await page.evaluate(() => JSON.parse(localStorage.getItem('cf:set-outbox:v1') || '[]'));
  out.push('outbox: ' + outbox.length + ' (id ' + (outbox[0]?.id ? 'set' : 'MISSING') + ')');
  let t = await main();
  out.push('row says: ' + (/On this phone/.test(t) ? 'On this phone' : 'MISSING'));
  await page.screenshot({ path: 'shots/offline-01-queued.png' });

  // 2 — finishing is refused while a set is waiting
  await btn('Finish workout early').click();
  await page.waitForTimeout(900);
  out.push('finish blocked: ' + (DB.workout_logs[0].completed_at ? 'NO — it finished' : 'yes'));

  // 3 — a reload keeps it (the phone died, the app reopened)
  await go('/member/track/session/L1');
  await page.waitForTimeout(600);
  t = await main();
  out.push('after reload: ' + (/On this phone/.test(t) ? 'still shown' : 'MISSING'));

  // 4 — signal back: it uploads with the same id, once
  globalThis.__offline = false;
  await page.evaluate(() => window.dispatchEvent(new Event('online')));
  await page.waitForTimeout(1800);
  out.push('server sets after online: ' + DB.workout_sets.length + ' id matches: ' + (DB.workout_sets[0]?.id === outbox[0]?.id));
  const left = await page.evaluate(() => JSON.parse(localStorage.getItem('cf:set-outbox:v1') || '[]').length);
  out.push('outbox after: ' + left);
  t = await main();
  out.push('row now: ' + (/On this phone/.test(t) ? 'STILL QUEUED' : 'Saved'));
  await page.screenshot({ path: 'shots/offline-02-synced.png' });
  return out.join('\n');
}
