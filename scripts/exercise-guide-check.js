/**
 * 0121 in the member app: the gym's own guide, where members meet it.
 *
 *   - An exercise the gym hid for itself leaves its members' list.
 *   - The exercise sheet plays the gym's video in the app, shows the gym's own
 *     steps in place of the library's, and the library's cues where the gym
 *     wrote none (an overlay's NULL means "use the library's").
 *   - Mid-workout, "How to" opens the same guide.
 *
 * Setup copied from workout-run-check.js (same session, gym and routine).
...
async (page) => {
  ...
  // 0121: the library carries starter cues/steps; the gym's overlay adds a
  // video and its own steps to the squat, and hides the leg press for itself.
  const EX = [
    { id: 'e1', name: 'Barbell Squat', muscle_group: 'legs', equipment: 'barbell', is_timed: false, is_active: true, sort_order: 1,
      cues: ['Library cue: chest up'], steps: ['Library step one.'] },
    ...
    { id: 'e3', name: 'Plank', muscle_group: 'core', equipment: 'bodyweight', is_timed: true, is_active: true, sort_order: 3,
      cues: ['Library cue: straight line'], steps: ['Hold it.'] },
  ];
  let n = 0;
  const uid = (p) => `${p}-${++n}`;
  const DB = {
    ...
    gym_exercise_media: [
      { exercise_id: 'e1', photo_url: null, video_url: 'https://youtu.be/dQw4w9WgXcQ', cues: null,
        steps: ['Gym step: unrack with both hands.', 'Gym step: sit between your heels.'], hidden: false, created_by: 'u9' },
      { exercise_id: 'e2', photo_url: null, video_url: null, cues: null, steps: null, hidden: true, created_by: 'u9' },
    ],
  ...
  // 1 - the exercise list: the gym hid Leg Press for itself.
  await go('/member/workouts?tab=exercises');
  let t = await text();
  out.push('library lists Barbell Squat: ' + (/Barbell Squat/.test(t) ? 'shown' : 'MISSING'));
  out.push('the gym hid Leg Press: ' + (/Leg Press/.test(t) ? 'MISSING (still listed)' : 'hidden'));
  out.push('squat marked as having a video: ' + ((await page.getByLabel('Has a video').count()) > 0 ? 'shown' : 'MISSING'));

  // 2 - the sheet: the gym's video and steps, the library's cues (the gym wrote none).
  ...
  out.push('gym video plays in the app: ' + (frame > 0 ? 'shown' : 'MISSING'));
  out.push("gym's own steps: " + (/Gym step: sit between your heels/.test(s) ? 'shown' : 'MISSING'));
  out.push("library steps replaced: " + (/Library step one/.test(s) ? 'MISSING (library steps still shown)' : 'replaced'));
  out.push('library cues where the gym wrote none: ' + (/Library cue: chest up/.test(s) ? 'shown' : 'MISSING'));
  out.push('no YouTube search when the gym has a video: ' + (/Search form videos on YouTube/.test(s) ? 'MISSING (search shown)' : 'none'));
  await page.screenshot({ path: 'shots/exercise-guide-sheet.png' });

  // 3 - mid-workout: "How to" on the squat opens the same guide.
  await go('/member/track/session/L1');
  await page.waitForTimeout(600);
  const howTo = page.getByRole('button', { name: 'How to' });
  out.push('How to button: ' + ((await howTo.count()) > 0 ? 'shown' : 'MISSING'));
  if (await howTo.count()) {
    await howTo.first().click();
    await page.waitForTimeout(1200);
    s = await sheetText();
    out.push('How to shows the gym video: ' + ((await page.locator('iframe[src*="youtube-nocookie.com"]').count()) > 0 ? 'shown' : 'MISSING'));
    out.push("How to shows the gym's steps: " + (/Gym step: unrack with both hands/.test(s) ? 'shown' : 'MISSING'));
    await page.screenshot({ path: 'shots/exercise-guide-howto.png' });
  }
