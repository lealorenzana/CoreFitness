/**
 * UI check (2026-09-17): header per tab, chat bubble, glass overlays, exercise
 * picker. Same fixture harness as nocturne-shots.js. Writes shots/ui-*.png.
 *
...
async (page) => {
  ...
  // An open session and a catalogue, so Track shows the set form and the picker.
  TABLES.workout_logs = [{ id: 'open1', member_id: 'm1', performed_on: dstr(0), activity: 'Weights',
    duration_minutes: null, notes: null, source: 'manual', completed_at: null, created_at: iso(0, 7, 0) },
    ...TABLES.workout_logs];
  TABLES.exercises = [
    ['Barbell Bench Press', 'chest', 'barbell'], ['Incline Dumbbell Press', 'chest', 'dumbbells'],
    ['Push-up', 'chest', ''], ['Deadlift', 'back', 'barbell'], ['Lat Pulldown', 'back', 'cable'],
    ['Pull-up', 'back', 'pull-up bar'], ['Back Squat', 'legs', 'barbell'], ['Leg Press', 'legs', 'machine'],
    ['Plank', 'core', ''],
  ].map(([name, g, eq], i) => ({ id: `x${i}`, name, muscle_group: g, equipment: eq, is_timed: name === 'Plank',
  ...
  const shot = (name) => page.screenshot({ path: `shots/ui-${name}.png`, animations: 'disabled' });

  // ── Header: identity on Today only ─────────────────────────────────────────
  for (const [name, path, wantIdentity] of [['today', '/member/home', true], ['train', '/member/book-class', false], ['you', '/member/membership', false]]) {
    await go(path);
    await shot(name);
    const r = await page.evaluate(() => ({
      identity: document.querySelectorAll('header button[aria-label="Your profile"]').length,
      settings: document.querySelectorAll('header button[aria-label="Settings"]').length,
      bell: document.querySelectorAll('header button[aria-label^="Notifications"]').length,
      assistantIcon: document.querySelectorAll('header button[aria-label^="AI assistant"]').length,
      bubble: document.querySelectorAll('.cursor-grab').length,
      titleWeight: getComputedStyle(document.querySelector('.screen-title')).fontWeight,
    }));
    check(`${name}: profile + greeting ${wantIdentity ? 'shown' : 'hidden'}`, (r.identity === 1) === wantIdentity, `identity=${r.identity}`);
    check(`${name}: bell and Settings in the header`, r.bell === 1 && r.settings === 1, `bell=${r.bell} settings=${r.settings}`);
    check(`${name}: no assistant icon in the header`, r.assistantIcon === 0);
    check(`${name}: chat bubble present`, r.bubble === 1, `bubble=${r.bubble}`);
    check(`${name}: title is bold like admin`, r.titleWeight === '700', r.titleWeight);
  }

  // ── Bubble opens the chat, closes, and leaves nothing eating taps ──────────
  await go('/member/home');
  const bubbleBox = await page.locator('.cursor-grab').boundingBox();
  const bar = await page.locator('nav[aria-label="Main"]').boundingBox();
  check('bubble sits above the bar', bubbleBox && bar && bubbleBox.y + bubbleBox.height <= bar.y + 1,
    bubbleBox && bar ? `bubble bottom ${Math.round(bubbleBox.y + bubbleBox.height)}, bar top ${Math.round(bar.y)}` : 'missing');
  await page.locator('.cursor-grab').click();
  await page.waitForTimeout(700);
  await shot('chat-open');
  check('bubble opens the assistant', await page.getByText('AI Assistant', { exact: true }).count() === 1);
  await page.getByRole('button', { name: 'Close' }).first().click();
  await page.waitForTimeout(700);
  await page.getByRole('button', { name: 'Train', exact: true }).click();
  await page.waitForTimeout(700);
  check('after closing the chat, the bar still takes taps', page.url().endsWith('/member/book-class'), page.url());

  // ── Glass panels ────────────────────────────────────────────────────────────
  const blurOf = (sel) => page.evaluate((s) => {
  ...
  await page.waitForTimeout(700);
  await shot('glass-notifications');
  check('notifications panel is glass', /blur/.test(await blurOf('[role="dialog"][aria-label="Notifications"]')));
  await page.keyboard.press('Escape');
  await go('/member/home');
  await page.getByRole('button', { name: 'More', exact: true }).click();
  await page.waitForTimeout(600);
  await shot('glass-more');
  check('More sheet is glass', /blur/.test(await blurOf('[role="dialog"][aria-label="Everything"]')));
  await go('/member/home');
  await page.locator('nav[aria-label="Main"] button[aria-label*="code"]').click();
  await page.waitForTimeout(900);
  await shot('glass-checkin');
  check('check-in sheet is glass', /blur/.test(await blurOf('[role="dialog"][aria-label="Check-in QR code"]')));

  // ── Track: the exercise picker ──────────────────────────────────────────────
  await go('/member/track');
  await shot('track-form');
  const native = await page.locator('select').count();
  check('no native select on Track', native === 0, `${native} select(s)`);
  await page.getByRole('button', { name: /Choose an exercise/ }).click();
  await page.waitForTimeout(700);
  await shot('track-picker');
  check('picker sheet opens, glass', /blur/.test(await blurOf('[role="dialog"][aria-label="Choose an exercise"]')));
  await page.getByLabel('Search exercises').fill('squat');
  await page.waitForTimeout(200);
  const hits = await page.locator('[role="dialog"][aria-label="Choose an exercise"] button[aria-pressed]').count();
  check('search narrows the list', hits === 1, `${hits} match(es) for "squat"`);
  await page.getByRole('button', { name: /Back Squat/ }).click();
  await page.waitForTimeout(700);
  const field = await page.getByRole('button', { name: /Back Squat/ }).count();
  const reps = await page.getByText('Reps', { exact: true }).count();
  check('choosing fills the field and shows the set inputs', field === 1 && reps === 1, `field=${field} reps=${reps}`);
  await shot('track-chosen');
