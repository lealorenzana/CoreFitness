/**
 * The admin operations screens (0095): Payments → End of day (expected from
 * the database, short count asks for a note), System (unpasted migrations,
 * grouped crash reports) and the Kiosk (typed code → welcome). Writes
 * shots/admin-cash.png, admin-system.png, admin-kiosk*.png.
 *
...
async (page) => {
  ...
  const TABLES = {
    ...
    cash_closeouts: [{ day: day(-1), expected: 4500, counted: 4400, difference: -100, payment_count: 3, refunds_out: 0,
      note: 'Gave change from the drawer', closed_at: iso(-1), previous: null }],
    client_errors: [
      { id: 'ce1', created_at: iso(0), app: 'member', route: '/member/home', message: "TypeError: Cannot read properties of null (reading 'firstName')", stack: 'at Home (Home.tsx:120)', user_agent: 'Android', build: 'index-abc.js' },
      { id: 'ce2', created_at: iso(0), app: 'member', route: '/member/book-class', message: "TypeError: Cannot read properties of null (reading 'firstName')", stack: 'at Home', user_agent: 'Android', build: 'index-abc.js' }],
  };
  const RPC = {
    cash_day_summary: [{ day: day(0), cash_in: 7500, refunds_out: 400, expected: 7100, payment_count: 5, closed: false,
      counted: null, difference: null, note: null, closed_by_name: null, closed_at: null }],
    migration_0095_applied: null, migration_0096_applied: null,
  ...
  await page.route(`**://${REF}.supabase.co/**`, async (route) => {
    ...
    if (pathname.startsWith('/rest/v1/rpc/')) {
      ...
      if (fn === 'migration_0095_applied' || fn === 'migration_0096_applied') return json({ code: 'PGRST202', message: 'no fn' }, 404);
  ...
  let t = await text();
  out.push('end of day: ' + grab(t, 'End of day', 200));
  await page.getByLabel('Amount counted').fill('7000');
  await page.waitForTimeout(300);
  out.push('short shown: ' + grab(await text(), 'Short', 20) + ' · note asked: ' + ((await page.getByLabel('Note').count()) ? 'yes' : 'NO'));
  ...
  await page.waitForTimeout(2500);
  await page.screenshot({ path: 'shots/admin-kiosk.png' });
  t = await text();
  out.push('kiosk: ' + grab(t, 'Check yourself in', 90));
  // Typed as stored: the kiosk must not change the case of what it sends (a
  // member id is lower-case; upper-casing it once made every lookup miss).
  await page.getByLabel('Check-in code').fill('QR-m1');
  await page.getByRole('button', { name: 'Go' }).click();
  await page.waitForTimeout(1800);
  // This harness filters on eq only, so every past visit counts as today:
  // 'already in' is the right answer here, 'Welcome' on a real first visit.
  const k = await text();
  out.push('kiosk result: ' + (/Welcome, Lea|already in, Lea/.exec(k)?.[0] ?? 'MISSING'));
  await page.screenshot({ path: 'shots/admin-kiosk-welcome.png' });
  return out.join('\n');
}
