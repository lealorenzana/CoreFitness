...
async (page) => {
  ...
  const SCREENS = [
    ['today',    '/member/home'],
    ['train',    '/member/book-class'],
    ['you',      '/member/membership'],
  ...
  const shoot = async (name, path) => {
    ...
    const m = await page.evaluate(() => {
      ...
      if (!main || !bar) return 'no main or bar';
      return `main.bottom=${Math.round(main.getBoundingClientRect().bottom)} bar.top=${Math.round(bar.getBoundingClientRect().top)}`;
    });
    out.push(`${name.padEnd(10)} ${m}`);
  };

  for (const [name, path] of SCREENS) await shoot(name, path);

  // Everything, opened from the bar.
