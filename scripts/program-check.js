/**
 * 0122 in the member app: following the gym's programs.
 *
 *   - the gym's published programs lead the workouts screen;
 *   - a Premium program the member's plan does not include is shown, locked,
 *     explained, and offers no Start;
 *   - starting a free program, then its day, opens the ordinary player running
 *     the gym's workout;
 *   - a finished day is ticked and the next one is Up next.
 *
...
async (page) => {
  ...
  const DB = {
    ...
    workout_logs: [{ id: 'L1', member_id: 'm1', activity: 'Leg day', routine_id: 'r1', created_at: new Date(Date.now() - 6 * 60000).toISOString(),
      completed_at: new Date().toISOString(), duration_minutes: 40, performed_on: dstr(0) }],
    workout_sets: [],
    ...
    saved_resources: [],
    // 0122: one free program and one Premium one. FEATURES below has no
    // premium_programs row, so this member's plan does not include Premium ones.
    gym_programs: [
      { id: 'pF', name: 'Starter Strength', description: 'Two weeks to learn the big lifts.', cover_url: null,
        level: 'beginner', weeks: 2, premium: false, published: true, hidden: false, created_at: iso(-5, 9, 0) },
      { id: 'pP', name: 'Advanced Block', description: 'Heavy and hard.', cover_url: null,
        level: 'advanced', weeks: 1, premium: true, published: true, hidden: false, created_at: iso(-4, 9, 0) },
    ],
    gym_program_days: [
      { id: 'd1', program_id: 'pF', week: 1, day: 1, workout_id: 'gw1' },
      { id: 'd2', program_id: 'pF', week: 1, day: 3, workout_id: 'gw1' },
    ],
    gym_workouts: [{ id: 'gw1', name: 'Leg Day A', notes: null, updated_at: iso(-5, 9, 0) }],
    gym_workout_items: [
      { id: 'gi1', workout_id: 'gw1', position: 0, exercise_id: 'e1', target_sets: 3, target_reps: 8, target_seconds: null, rest_seconds: 90 },
      { id: 'gi2', workout_id: 'gw1', position: 1, exercise_id: 'e3', target_sets: 2, target_reps: null, target_seconds: 20, rest_seconds: 30 },
    ],
  ...
  const embed = (t, r) => {
    if (t === 'gym_program_days') {
      return { ...r, gym_workouts: { name: (DB.gym_workouts.find((w) => w.id === r.workout_id) || {}).name } };
    }
    if (t === 'gym_workouts') {
      return { ...r, gym_workout_items: DB.gym_workout_items.filter((i) => i.workout_id === r.id)
        .map((i) => ({ ...i, exercises: EX.find((e) => e.id === i.exercise_id) ?? null })) };
    }
  ...
  let enrolled = null;
  const FN = {
    start_program: (b) => { enrolled = b.p_program; return 'en1'; },
    leave_program: () => { enrolled = null; return null; },
    start_program_day: (b) => {
      const d = DB.gym_program_days.find((x) => x.id === b.p_day);
      const w = DB.gym_workouts.find((x) => x.id === d.workout_id);
      const id = 'PL' + (++n);
      DB.workout_logs.push({ id, member_id: 'm1', activity: w.name, routine_id: null, gym_workout_id: w.id,
        program_day_id: d.id, created_at: new Date().toISOString(), completed_at: null, duration_minutes: null, performed_on: dstr(0) });
      return id;
    },
    program_progress: () => !enrolled ? [] : DB.gym_program_days.filter((d) => d.program_id === enrolled)
      .sort((a, b) => a.week - b.week || a.day - b.day)
      .map((d) => ({ program_id: enrolled, program_name: 'Starter Strength', day_id: d.id, week: d.week, day: d.day,
        workout_id: d.workout_id, workout_name: 'Leg Day A',
        done: DB.workout_logs.some((l) => l.program_day_id === d.id && l.completed_at) })),
  ...
  // 1 - the gym's programs, first on the workouts screen.
  await go('/member/workouts');
  let t = await text();
  out.push("gym programs listed: " + (/Your gym's programs/.test(t) && /Starter Strength/.test(t) ? 'shown' : 'MISSING'));
  out.push('Premium program shown with a lock: ' + (/Advanced Block/.test(t) && /Premium/.test(t) ? 'shown' : 'MISSING'));

  // 2 - locked: explained, never startable.
  await page.getByRole('button', { name: /Advanced Block/ }).first().click();
  await page.waitForTimeout(1200);
  t = await text();
  out.push('locked program explains itself: ' + (/Part of Premium/.test(t) ? 'shown' : 'MISSING'));
  out.push('and offers no Start: ' + ((await page.getByRole('button', { name: 'Start this program' }).count()) === 0 ? 'none' : 'MISSING (Start shown)'));
  await page.screenshot({ path: 'shots/program-locked.png' });

  // 3 - a free program: start it, then run its next day in the player.
  await go('/member/program/pF');
  t = await text();
  out.push('program weeks: ' + (/Week 1/.test(t) && /Leg Day A/.test(t) ? 'shown' : 'MISSING'));
  await page.getByRole('button', { name: 'Start this program' }).click();
  await page.waitForTimeout(1200);
  out.push('start sent: ' + (CALLS.start_program?.p_program === 'pF' ? 'pF' : 'MISSING'));
  t = await text();
  out.push('up next after starting: ' + (/Up next/.test(t) ? 'shown' : 'MISSING'));
  await page.screenshot({ path: 'shots/program-following.png' });
  await page.getByRole('button', { name: /Start Leg Day A/ }).click();
  await page.waitForTimeout(2500);
  out.push('day sent: ' + (CALLS.start_program_day?.p_day === 'd1' ? 'd1' : 'MISSING'));
  out.push('opens the player: ' + (/\/member\/track\/session\/PL/.test(page.url()) ? 'yes' : 'MISSING ' + page.url()));
  t = await text();
  out.push("player runs the gym's workout: " + (/Barbell Squat/.test(t) ? 'Barbell Squat' : 'MISSING'));

  // 4 - finishing it ticks the day.
  DB.workout_logs.forEach((l) => { if (l.program_day_id === 'd1') l.completed_at = new Date().toISOString(); });
  await go('/member/program/pF');
  out.push('finished day ticked: ' + ((await page.getByLabel('Done').count()) === 1 ? 'ticked' : 'MISSING'));
  t = await text();
