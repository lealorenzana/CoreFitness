/**
 * The full-screen routine run (GuidedWorkout, 2026-09-19) against a stateful
 * fixture: no tab bar, the focus set's steppers, logging writes a set, the rest
 * takeover (+15 s, hide to a pill, skip), "beat last time", the exercise list,
 * a timed hold, and the finish screen. Writes shots/run-NN-*.png.
 *
...
async (page) => {
  ...
  const skipRest = async () => {
    if (await page.getByRole('dialog', { name: 'Rest' }).count()) await btn('Skip rest').click();
    else if (await btn(/^Rest \d/).count()) { /* manual rest link, not running */ }
    await page.waitForTimeout(250);
  };

  // 1 — full screen, the exercise, no tab bar
  await go('/member/track/session/L1');
  await page.waitForTimeout(600);
  await shot('01-open');
  let t = await text();
  out.push('exercise: ' + (/Barbell Squat/.test(t) ? 'Barbell Squat' : 'MISSING'));
  out.push('tab bar: ' + ((await page.getByRole('link', { name: 'Today' }).count()) ? 'STILL SHOWN' : 'hidden'));
  out.push('status: ' + (/Up next/i.test(t) ? 'Up next' : '?'));

  // 2 — start, step the weight up, log
  await btn('Start exercise').click();
  await page.waitForTimeout(500);
  await shot('02-focus');
  await btn('More kg').click();
  await btn('More kg').click();
  out.push('kg after +2.5 ×2: ' + await page.getByLabel('kg', { exact: true }).inputValue());
  await btn('Log set 1').first().click();
  await page.waitForTimeout(900);
  out.push('set written: ' + JSON.stringify(DB.workout_sets.map((s) => [s.set_number, s.weight_kg, s.reps])));
  const rest = page.getByRole('dialog', { name: 'Rest' });
  out.push('rest takeover: ' + ((await rest.count()) ? 'shown' : 'MISSING'));
  await shot('03-rest');
  t = (await rest.innerText().catch(() => '')).replace(/\s+/g, ' ');
  out.push('rest text: ' + t.slice(0, 90));
  await btn('+15 s').click();
  await page.waitForTimeout(300);
  out.push('after +15: ' + ((await rest.innerText()).match(/\d:\d\d/) || ['?'])[0]);
  await btn('Hide the rest timer').click();
  await page.waitForTimeout(400);
  await shot('04-rest-pill');
  t = await text();
  out.push('beat badge: ' + (/Beat last time/.test(t) ? 'shown' : 'MISSING'));
  await page.locator('footer button').filter({ hasText: 'Rest' }).first().click();
  await page.waitForTimeout(300);
  await btn('Skip rest').click();
  await page.waitForTimeout(300);

  // 3 — the rest of the squats, then the next exercise
  await btn('Log set 2').first().click(); await page.waitForTimeout(700); await skipRest();
  await btn('Log set 3').first().click(); await page.waitForTimeout(700); await skipRest();
  await shot('05-exercise-done');
  await page.getByRole('button', { name: /Next exercise/ }).last().click();
  await page.waitForTimeout(900);
  await shot('06-leg-press');
  t = await text();
  out.push('moved to: ' + (/Leg Press/.test(t) ? 'Leg Press' : 'MISSING'));

  // 4 — the exercise list
  await btn('All exercises').click();
  await page.waitForTimeout(500);
  await shot('07-overview');
  await page.getByRole('dialog').getByRole('button', { name: /Plank/ }).click();
  await page.waitForTimeout(700);

  // 5 — a timed hold
  await btn('Start exercise').click();
  await page.waitForTimeout(300);
  await btn('Start hold').click();
  await page.waitForTimeout(2300);
  await shot('08-hold');
  await page.getByRole('button', { name: /Stop and log/ }).click();
  await page.waitForTimeout(900);
  const hold = DB.workout_sets.filter((s) => s.duration_seconds != null);
  out.push('hold logged: ' + JSON.stringify(hold.map((s) => s.duration_seconds)));
  await skipRest();

  // 6 — finish early (leg press untouched) → finish screen
  await btn('Finish workout early').click();
  await page.waitForTimeout(1200);
  await shot('09-finish');
  t = await text();
  out.push('finish: ' + (/Workout complete/i.test(t) ? 'shown' : 'MISSING') + ' | ' + (/(\d+) sets? beat last time/.exec(t) || ['no beat line'])[0]);
  out.push('log closed: ' + (DB.workout_logs[0].completed_at ? 'yes' : 'NO'));
  const errors = [];
