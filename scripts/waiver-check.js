/**
 * The gym's waiver and the PAR-Q (0119), from the member's side.
 *
 * Built on language-check.js's session and network fixture (same member, same
 * routing) so the only new thing here is the waiver itself. Asserts what a
 * build cannot: that all seven questions render, that Sign stays disabled until
 * every one is answered and the box is ticked, that what is sent is exactly the
 * answers given, and that a "yes" produces a referral rather than a block.
 * Writes shots/waiver-*.png.
 *
...
async (page) => {
  ...
  const DB = {
    profiles: [ME],
    member_profiles: [{ profile_id: 'm1', experience_level: 'beginner', qr_code: 'QR', created_at: iso(-90, 9, 0), profiles: ME, preferred_language: 'en' }],
  ...
  const CALLS = {};
  const WAIVER = { waiver_id: 'w1', version: 2, title: 'Harbour waiver',
    body: 'I understand that exercise carries a risk of injury.', accepted_at: null,
    required: true, flagged: false };
  const PARQ = [
    ['heart_condition', 'Has a doctor ever said you have a heart condition?'],
    ['chest_pain_active', 'Do you feel pain in your chest when you do physical activity?'],
    ['chest_pain_rest', 'In the past month, have you had chest pain at rest?'],
    ['balance', 'Do you lose your balance because of dizziness?'],
    ['bone_joint', 'Do you have a bone or joint problem?'],
    ['medication', 'Is your doctor prescribing drugs for blood pressure or a heart condition?'],
    ['other_reason', 'Any other reason you should not do physical activity?'],
  ].map(([key, question], i) => ({ key, question, sort_order: i + 1 }));
  const FN = {
    my_waiver_status: () => [WAIVER],
    parq_questions: () => PARQ,
    accept_waiver: (b) => {
      const yes = Object.values(b.p_answers || {}).some(Boolean);
      WAIVER.accepted_at = new Date().toISOString();
      WAIVER.flagged = yes;
      return yes;
    },
  ...
  await go('/member/waiver');
  let text = (await page.locator('main').innerText()).replace(/\s+/g, ' ');
  out.push('waiver shown: ' + (/Harbour waiver/.test(text) && /risk of injury/.test(text) ? 'yes' : 'MISSING'));
  const qCount = PARQ.filter((q) => text.includes(q.question)).length;
  out.push('PAR-Q questions on screen: ' + qCount + (qCount === 7 ? '' : ' MISSING some'));

  const signBtn = page.getByRole('button', { name: /^Sign$/ });
  // Nothing answered, box unticked: the button must not work.
  out.push('sign disabled before answering: ' + ((await signBtn.isDisabled()) ? 'yes' : 'NO'));

  // Answer six "No" and one "Yes" (bone_joint) -- a referral, never a block.
  const noButtons = page.getByRole('button', { name: 'No', exact: true });
  const yesButtons = page.getByRole('button', { name: 'Yes', exact: true });
  for (let i = 0; i < 7; i++) {
    if (i === 4) await yesButtons.nth(i).click(); else await noButtons.nth(i).click();
  }
  // Every question shows its answer as selected -- read from aria-pressed, not
  // from a screenshot, where a dim border on the last row looked unselected.
  const pressed = await page.locator('main button[aria-pressed="true"]').count();
  out.push('answers shown as selected: ' + pressed + (pressed === 7 ? '' : ' MISSING some'));
  out.push('sign still disabled until the box is ticked: ' + ((await signBtn.isDisabled()) ? 'yes' : 'NO'));
  await page.getByRole('checkbox').check();
  out.push('sign enabled once everything is done: ' + ((await signBtn.isDisabled()) ? 'NO' : 'yes'));
  await shot('waiver-01-answered');
  await signBtn.click();
  await page.waitForTimeout(1200);

  const sent = CALLS.accept_waiver?.p_answers ?? {};
  const keys = Object.keys(sent);
  out.push('answers sent: ' + keys.length + ' of 7'
    + (keys.length === 7 && sent.bone_joint === true && sent.heart_condition === false ? '' : ' MISSING the right ones'));
  text = (await page.locator('main').innerText()).replace(/\s+/g, ' ');
  out.push('after signing: ' + (/You signed version 2/.test(text) ? 'signed' : 'MISSING'));
  out.push('a yes refers, it does not block: '
    + (/talk to the front desk/.test(text) && /does not stop you training/.test(text) ? 'yes' : 'MISSING'));
  await shot('waiver-02-signed');
