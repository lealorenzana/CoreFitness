/**
 * Opens every admin route against the fixture and reports, per screen, any
 * crash (the ErrorBoundary), error sentence on the screen ("Could not load",
 * NaN, Invalid Date, [object Object]) or console error. The broad net; the
 * feature checks go deep. Playwright runner's `filename`, admin dev server on :5174.
 */
async (page) => {
  ...
  // Every admin route, looking for crashes, error screens and console errors.
  const ROUTES = ['/dashboard', '/members', '/members/m1', '/attendance', '/attendance-history', '/analytics', '/retention',
    '/revenue', '/payments', '/membership-plans', '/resources', '/trainers', '/schedule', '/bookings', '/events',
    '/notifications', '/activity', '/achievements', '/exercises', '/rewards', '/challenges', '/credentials', '/settings',
    '/system', '/kiosk'];
  const BAD = /Something went wrong|Couldn'?t load|Could not load|Failed to load|failed to load|is not a function|Cannot read prop|undefined is not|NaN|\[object Object\]|Invalid Date/;
  let errs = [];
  page.on('pageerror', (e) => errs.push('pageerror: ' + e.message.slice(0, 160)));
  page.on('console', (m) => { if (m.type() === 'error' && !/Failed to load resource|favicon|net::ERR|404|DevTools/.test(m.text())) errs.push('console: ' + m.text().slice(0, 160)); });
  for (const r of ROUTES) {
    ...
    // A button or link inside another — invalid HTML, and the inner tap can fire the outer.
    const nested = await page.evaluate(() => document.querySelectorAll('button button, button a[href], a[href] button, a[href] a[href]').length);
    if (nested) errs.push(`NESTED interactive elements: ${nested}`);
    out.push(`${r.padEnd(20)} ${url !== r ? '→ ' + url + ' ' : ''}${bad ? 'SCREEN: "' + t.slice(Math.max(0, bad.index - 60), bad.index + 80) + '"' : 'ok'}${errs.length ? ' | ' + [...new Set(errs)].slice(0, 3).join(' || ') : ''}`);
    await page.screenshot({ path: `shots/sweep${r.replace(/\//g, '-')}.png` });
  }
  // A screen fails when it shows an error sentence or logs one; the rest end in "ok".
  return { failures: out.filter((l) => !/ ok$/.test(l)), report: out.join('\n') };
}
