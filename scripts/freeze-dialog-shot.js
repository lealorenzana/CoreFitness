async (page) => {
  ...
  // The freeze dialog, now showing the yearly ceiling next to the monthly one.
  await page.goto('http://localhost:5174/members', { waitUntil: 'domcontentloaded' });
  await page.waitForTimeout(2500);
  const freeze = page.locator('[data-tip="Freeze membership"]').first();
  if (await freeze.count()) {
    await freeze.click();
    await page.waitForTimeout(1800);
    await page.screenshot({ path: 'shots/21-freeze-yearly-ceiling.png' });
    out.push('21-freeze-yearly-ceiling');
    out.push((await page.locator('body').innerText()).replace(/\s+/g, ' ').slice(0, 1200));
  } else {
    out.push('SKIPPED — no Freeze control found on the members list');
  }
