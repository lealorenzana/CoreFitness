async (page) => {
  ...
  // A month of check-ins: busy 6am and 6pm, a lunch bump, Sunday quiet.
  const HOURS = [6, 6, 7, 9, 12, 13, 17, 18, 18, 19, 20];
  let n = 0;
  for (let d = 1; d <= 29; d++) {
    const dow = new Date(Date.now() - d * 86400000).getDay();
    const perDay = dow === 0 ? 3 : dow === 6 ? 7 : 10 + (d % 4);
    for (let k = 0; k < perDay; k++) {
      const h = HOURS[(k * 7 + d) % HOURS.length];
      TABLES.attendance.push({ id: `ax${n++}`, member_id: 'm1', gym_id: null,
        check_in_time: iso(-d, h, (k * 13) % 60), method: 'qr', recorded_by: 'u1' });
    }
  }
  ...
  await page.screenshot({ path: 'shots/41-dashboard-heatmap.png' });
  const info = await page.evaluate(() => {
    const h3 = [...document.querySelectorAll('h3')].find((x) => x.textContent.trim() === 'Member activity');
    const tile = h3 && h3.closest('[style*="grid-column"]');
    const cells = tile ? [...tile.querySelectorAll('[data-tip*="check-in"]')] : [];
    const main = document.querySelector('main');
    return {
      heatmapTile: !!tile,
      subtitle: h3 ? h3.nextElementSibling?.textContent : null,
      cells: cells.length,
      filledCells: cells.filter((c) => c.textContent.trim() !== '').length,
      memberStatisticsGone: !document.body.innerText.includes('Member statistics'),
      busiestDayTile: document.body.innerText.includes('Busiest day'),
      mainOverflowPx: main ? main.scrollHeight - main.clientHeight : null,
      checkins: document.querySelectorAll('x').length,
    };
  });
