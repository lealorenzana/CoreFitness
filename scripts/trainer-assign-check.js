/**
 * 0122, the coach's side: a trainee's program, and assigning one.
 *
 * The trainee sheet says what they follow (or that they follow nothing), marks
 * Premium programs in the picker, sends assign_program for the chosen one, and
 * then shows it. Whether a coach MAY assign — only to their own trainees, and
 * a Premium program only to a plan that includes it — is the database's rule,
 * proven in scripts/sql/programs.mjs.
 *
...
async (page) => {
  ...
  await page.waitForTimeout(1500);
  out.push('assign sent: ' + (CALLS.assign_program?.p_member === 'mb1' && CALLS.assign_program?.p_program === 'pF' ? 'mb1 -> pF' : 'MISSING ' + JSON.stringify(CALLS.assign_program)));
  t = await text();
  out.push('the sheet then shows it: ' + (/Starter Strength · 0 of 2 days done/.test(t) ? 'shown' : 'MISSING'));
  await page.screenshot({ path: 'shots/trainer-assign-program.png' });
