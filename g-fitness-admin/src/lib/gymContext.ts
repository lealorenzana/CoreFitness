 ...
 *
 * Memory only, cleared on sign-out and on a gym switch (CLAUDE.md: per-member
 * caches never live in localStorage).
 *
 * The member app has its own copy — diff before you copy (CLAUDE.md). The only
 * difference is homeFor(): this app is where an owner and their front desk work.
 */
...
/** Where a role lands in this app. Members and coaches use the phone app. */
export function homeFor(role: GymRole | null): string | null {
  return role === 'admin' || role === 'staff' ? '/admin/dashboard' : null;
}
