...
export const SharedStorage = {
  ...
  getMember: (memberId: string) => {
    ...
    return members.find((m: any) => m.id === memberId || m.qrCode === memberId);
  },
  ...
  updateMember: (memberId: string, updates: any) => {
    ...
    const updatedMembers = members.map((m: any) => 
      (m.id === memberId || m.qrCode === memberId) ? { ...m, ...updates } : m
    );
  ...
  getMemberBookings: (memberId: string) => {
    const bookings = SharedStorage.getBookings();
    return bookings.filter((b: any) => b.memberId === memberId);
  },
  ...
  getMemberPayments: (memberId: string) => {
    const payments = SharedStorage.getPayments();
    return payments.filter((p: any) => p.memberId === memberId);
  },
...
export const initializeSharedData = () => {
  ...
  if (!localStorage.getItem(STORAGE_KEYS.MEMBERS)) {
    // Will be populated from MEMBERS data on first load
    SharedStorage.setMembers([]);
  }
  if (!localStorage.getItem(STORAGE_KEYS.BOOKINGS)) {
    SharedStorage.setBookings([]);
  }
  if (!localStorage.getItem(STORAGE_KEYS.PAYMENTS)) {
    SharedStorage.setPayments([]);
  }
  if (!localStorage.getItem(STORAGE_KEYS.ATTENDANCE)) {
    SharedStorage.setAttendance([]);
  }
};
