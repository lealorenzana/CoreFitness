...
legacy — deliberately left alone. `supabase/` holds 122 migrations, RLS policies and eight Edge
...
- **An identifier is unique because a constraint says so**, not because a formula looks unlikely to repeat; **clocks
  ...
  **jsQR** over the full frame, **never a crop**. **A control writing a flag nothing reads is a lie**; **a rule
  enforced only in SQL the user cannot read ambushes them** (0017 → 0041). **Every per-member cache is cleared in ONE list, `lib/memberCaches.ts`**, called by `logout()` and `switchGym()` — two hand-kept lists had drifted and the achievement catalogue (a per-gym table) had no clearer at all, so it survived one person signing out and the next signing in. **Admin had the same bug** (2026-09-26: Ana signed in after G Fitness's owner and her sidebar wore G Fitness's name and logo) — `lib/adminCaches.ts`, and in both apps **a listener on `onAuthStateChange` clears them whenever the account changes**, so no sign-out path can forget (`admin-account-swap-check.js`). **Per-user state never lives in
...
**Both apps read `my_gym_context()`/`gym_people` (0104), never the legacy columns.** **`staff`** is front desk (0011/0012): payments, check-ins, not pricing, trainers,
...
## Roadmap
**0001–0121 are all live** (verified by `probe-migrations.py`, 2026-09-26; **0122 is pasted by hand**). **A gym program day is an ordinary `workout_logs` row** (0122: `gym_workout_id`, `program_day_id`, the same player), so points/badges/history need nothing new and "done" is computed by `program_progress()`, never stored; a Premium program's row stays readable so the lock explains itself (`plan_allows(member,'premium_programs')`), its days do not; owners build programs, trainers build their own workouts and assign only to their own trainees, and `program_enrolments` has no write policy. **A gym's content never goes on a shared row** (0121): the 36 standard exercises are one row each for every gym, so a gym's photo/video/cues/steps and its "hide this" live in the per-gym `gym_exercise_media` overlay, the library is **the platform's alone** to curate, and a content photo needs a counted slot (`reserve_gym_photo()`, `max_photos`). **Storage was the tenancy gap** (0120): the three buckets kept pre-tenancy policies asking only "admin?", so any gym's owner read every trainer's credential files and could delete any gym's logo — **a file belongs to a gym because its path says so** (`media/gyms/<gym_id>/…`), proven by `storage-tenancy.mjs` — tenancy: [TENANCY](docs/TENANCY.md). Four apps: member, admin (Vercel), platform (localhost), site. **A new gym gets Gym #1's rules and none of its identity**: `approve-gym` makes its owner (a temporary password, shown once — nothing here sends mail), who sets the gym up at `/admin/setup`. **The platform sells plans to gyms exactly as a gym sells them to members** — `platform_plans`/`gym_plan_allows()` mirror `membership_plans`/`plan_allows()`, and a plan limit is a trigger on `gym_roles`, not a label. **Three gates, and they differ on screen**: the gym's plan and the gym's own `gym_modules` both **hide** (a member can never buy either), the member's plan **locks and explains** (0049). **A gym's stored `status` is only the platform's decision** — everything derivable (no owner, not set up, on trial, overdue) is derived once by `gym_state()` (0111), never a second column. **An invitation is addressed**: accepting needs the email it was sent to, and `gym_invitations` has RLS on with **no permissive policy** because a token is a credential. **Support access is granted by the gym, never taken** (0113): one `gym_writable()` false makes all ~50 gym tables read-only, so it is provable rather than conventional. **A setting the owner can reach once is not a setting** — 0112's second colour was set only by the setup wizard, so 0114 moved both colour roles, the logo and the gym's own nouns onto **Your app**, and Settings points there rather than offering a second picker. A gym renames `member(s)`/`trainer(s)`/`class(es)` (0114) and **moves its own `/join/<slug>`**; a missing word means the English one, so a gym that changed nothing reads unchanged. **`/join/<slug>` asks `gym_by_slug()`, never `list_gyms()`**, which lists only `open` gyms. A rename is applied **per key against the defaults** — `trainers` defaults to "coaches", so "did they rename anything" renamed a row on every gym; admin **Your app** prints a **join poster** (QR + code, portalled to `<body>` so one `@media print` rule hides the dashboard). **Supabase's advisor flags seven "Security Definer View" as CRITICAL and they must stay that way** — each filters to `current_gym_id()` itself and each is an aggregate over rows the caller may not read singly (flipping `class_availability` keeps all 72 rows and counts **0** bookings, so **count rows and you will ship a lie**); 0115 adds the missing `security_barrier` and revokes `anon`, and `views_without_protection()` is the rule. **`live-db.mjs` used to re-grant `anon` after the last migration**, so every `revoke ... from anon` was untestable — it uses `alter default privileges` now. **A gym's logo existed from 0067 and no member ever saw it**: four migrations carried it to the phone and no component drew it (0116 — `my_gyms()` carries it, `GymMark` renders it, Today and the picker show it; a gym with none gets a monogram in its own colour, **never the Core Fitness mark**). — admin **System** lists what is not live. `lib/memberDataExport.ts` is identical in both apps. Verify with `python scripts/probe-migrations.py` (REST, no DB credentials)
...
- **Demo data may be live**: `scripts/demo-data/` seeds 150 members, payments, classes, coaches, events and rewards (ids `5eed____-0000-4000-8000-`); part 2 goes in as `part2/` one file at a time. **0117 moved the removal out of the 166-line paste into `remove_demo_data()`** — platform app, typed confirmation, irreversible; the script file now only calls it, because two copies of that delete is how the stale one gets pasted. A gym owner sees the **count** on System and no button (it reaches `profiles`/`auth.users`, which are not gym-scoped). Proof before the button: `scripts/sql/demo-removal.mjs`. **Dashboard figures include it until removed.** 0080 hid it from members via `sees_demo_data()`; **0084 shows it to everyone** (that one function body), gives the 12 demo coaches hours and un-retires the demo timetable.
- **Staff approve registrations through 0078's `set_account_status()`**, one transition only. **A member asks to freeze or cancel; the desk still does it** (0118, the same shape as 0091's renewal request) — **granting is a trigger on `membership_events`, never a button**, and the member sees `refund_quote()` (0070/0073) *before* asking, which no member-facing screen had ever called. **A waiver's published words are immutable** (0119, a trigger that refuses even the table owner) — editing makes the next version, and every signature points at the words it was shown. The PAR-Q is **fixed in SQL** (a gym shortening it is a liability); a yes **refers, never blocks or changes training**; health answers are **desk-only, never trainers** (Privacy says so, the harness asserts it). `waiver_required` gates **booking only** — never check-in, the free library, or a desk booking.
