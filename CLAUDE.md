...
## Roadmap
**0001–0094 are live; 0095–0096 await pasting** (0095: crash reports, cash close-out, trainer totals, language; 0096: class waitlist) — admin **System** (`/system`) lists what is not live. `lib/memberDataExport.ts` is identical in both apps. Verify with `python scripts/probe-migrations.py` (REST, no DB credentials)
